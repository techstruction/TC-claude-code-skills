...
> **⚠️ Local machine requirement:** Browser-mode commands (Twitter, Reddit, YouTube, Bilibili, etc.) require a **local Chrome browser** open and logged in to the target site, plus the **autocli Chrome Extension** installed. This skill will NOT work on headless servers or remote machines without a display. Public-mode commands (HackerNews, Wikipedia, Arxiv, BBC, StackOverflow, etc.) work anywhere without a browser.
...
**Chrome Extension (required for browser-mode):** Download from https://github.com/nashsu/AutoCLI/releases/latest
